...
class Ambiente():
    ...
    def __init__(self, S0 = 10, mu = 0.1, kappa = 5, theta = 1, sigma = 0.1, lambd = 0.1, t0 = 0, t = 1, T = 3600, inv = 20): #T = 1.0, M = 7200, I = 1_000 mu = 0.01
...
class Agente():
    ...
    def __init__(self, inventario, numTrain):
        ...
        self.gamma = 1#0.99   ##############################################
        self.timestep = 0
        self.update_target_steps = 10_000
        self.lots_size = 100
...
#
        ...
        return loss.cpu().item(), grad_norm, self.matrix
    ...
    def step(self, inv, tempo, data):
        ...
        if len(self.memory) < self.batch_size:
            ...
            return 1, 0, np.zeros((21,5)), new_inv, x, r
...
if __name__ == '__main__': 
    ...
    def doTrain(age, numIt = 200):
        ...
        for j in tqdm(range(numIt)):
            ...
            for i in tqdm(range(slices)):
                ...
                loss, grad, state, new_inv, action, reward = age.step(inv, tempo, dati) 
                inv = new_inv 
                tempo += 1
        ...
        return (act_mean, act_sd, act_hist, loss_mean, loss_sd, rew_mean, rew_sd, loss_hist, rew_hist, state)
    ...
    def doTest(age, numIt = 100):
        ...
        mean_list.append(stat.mean(np.asarray(transaction_cost_balance)/100))
        std_list.append(stat.stdev(np.asarray(transaction_cost_balance)/100))
        
        
        #performance_list.append(performance(transaction_cost_balance))
        
        return mean_list, std_list, act, *doAve(act), *doAve(re), re, transaction_cost_balance# np.asarray(transaction_cost_balance).mean(), np.asarray(transaction_cost_balance).std()transaction_cost_balance#*doAve(mean_list)
    ...
    def run(test = False):
        ...
        act_mean, act_sd, act_hist, loss_mean, loss_sd, rew_mean, rew_sd, loss_hist, rew_hist, state = doTrain(age, numIt)

        heat = get_heatmap(age)

        if test == True:
            pel, sdPeL, azioni, azioni_med, sdaz, ricompensa, sdRic, re, tr = doTest(age, numTr) #doAve(mean_list), act, doAve(act), doAve(re)
            #return pel, azioni, azioni_med, ricompensa#a, a_var, a_t, b, b_var, c, c_var, pel, azioni, azioni_med, ricompensa, loss_hist, rew_hist
            np.savetxt('data.csv', azioni, delimiter = ',')

            print('average PandL =', pel, ', PandL sd = ', sdPeL ,
            ...
            plt.ylabel('rewards')
            plt.show()
            plt.hist(   np.asarray(tr).reshape(-1,5).mean(0))
