    ...
    def updn(self, seed = 14, numIt=10):
        ...
        for i in range(N):
            ...
            if np.random.binomial(1, 0.5) == 1 :
                sgn = -self.mu
            elif np.random.binomial(1, 0.5) == 0:
                sgn = self.mu
...
class Agente():
    ...
    def __init__(self, inventario, numTrain):
        ...
        self.gamma = 1#0.99   ##############################################
        self.timestep = 0
        self.update_target_steps = 1_000
        self.lots_size = 100
...
if __name__ == '__main__': 
    ...
    def doTest(age, numIt = 100):
        ...
        return mean_list, std_list, act, *doAve(act), *doAve(re), re, transaction_cost_balance, states# np.asarray(transaction_cost_balance).mean(), np.asarray(transaction_cost_balance).std()transaction_cost_balance#*doAve(mean_list)
    ...
    def run(test = False):
        ...
        if test == True:
            pel, sdPeL, azioni, azioni_med, sdaz, ricompensa, sdRic, re, tr, states = doTest(age, numTr) #doAve(mean_list), act, doAve(act), doAve(re)
